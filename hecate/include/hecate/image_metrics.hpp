...
#define VALIDATE(x) ( std::isnan(x) ) ? 0 : x
    ...
    inline double calc_uniformity_wrapper(const Mat& gray_img)
    {
        return hecate::calc_uniformity(gray_img);
    }
    ...
    inline double calc_entropy_wrapper(const Mat &gray_img) {
        return hecate::calc_entropy(gray_img);
    }
